...
public class UIManager11 : MonoBehaviour {
	...
	public void but(){
		...
		if(slider.value<=25){
			if(UIManager2.coin_score>=95){
				pause2.SetActive(false);
		        Time.timeScale=1;
		        aud[0].UnPause();
		        slider1.value=0;
		       ObscuredPrefs.SetInt("Score",UIManager2.coin_score-=95);
			}
			else {
				StartCoroutine(tabl());
			}
		...
		if(slider.value>25){
			if(UIManager2.coin_score>=100){
				pause2.SetActive(false);
		        Time.timeScale=1;
		        aud[0].UnPause();
                slider1.value=0;
		        ObscuredPrefs.SetInt("Score",UIManager2.coin_score-=100);
	...
	IEnumerator wins (){
		...
		Time.timeScale=0;
		yield return new WaitForSecondsRealtime(1f);
		if(ObscuredPrefs.GetString("played10")!="On"){
			ObscuredPrefs.SetInt("Score",UIManager2.coin_score+=30);
		}
		yield return new WaitForSecondsRealtime (0.2f);
		ObscuredPrefs.SetString ("played10","On");
